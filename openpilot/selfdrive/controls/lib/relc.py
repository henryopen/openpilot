"""Hold off a lane change while the road edge on that side is too close.

Ported from sunnypilot (originally rav4kumar's), thresholds and timings unchanged.
sunnypilot published the result on a custom message; master has no room for one, so
this hands it straight back to the caller instead.
"""
import numpy as np
...
EDGE_CLEAR_TIME = 0.3
MIN_SPEED = 30 * CV.KPH_TO_MS   # matches the lane change's own minimum
...
class RoadEdgeLaneChangeController:
  ...
  def update_from_model(self, modelv2, v_ego):
