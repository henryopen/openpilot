...
| 第二則訊息 | 🔍 掃完了：有一個**動/靜分類欄位**（Intel bit 23，3 bit），但**與 `V_ABS` 冗餘**，見下 | 見下 |
| 第三則訊息 | 🔍 掃完了，沒有可用的東西 | 同下 |
| `0x25a–0x25e` | 🔍 掃完了，沒有可用的東西 | 同下 |
...
### 第二、三則訊息與五則彙總：掃完了，不用再挖（2026-09-04）

兩種位元組順序（Motorola 與 Intel）、長度 8–16 bit、有號無號全掃，對「前車對地速 / 前車橫向 /
距離 / 本車速」四個基準：**最高相關只有 0.42–0.64**，而同樣的掃描在第一則訊息上會給出
距離 r=1.00、速度 r=0.95。在上千個候選裡挑出來的 0.6 是雜訊。

線性掃描看不到「類別碼」，所以另外用分類能力測了一次（18 萬個目標樣本，以 `V_ABS` 當真值）：

| | 分類「靜止 vs 在動」的正確率 |
|---|---|
| 什麼都不看，全猜多數 | 81.3% |
| **B 第二則，Intel bit 23+3** | **94.9%** |
| C 第三則，最好的欄位 | 84.1%（≈ 沒有資訊） |

B 那個欄位是真的，三天一致：**值 2 和 3 代表在動**（判為靜止的比例 0–2%，對地速中位 4.9–13.3 m/s），
其餘六個值以靜止居多。看起來就是雷達自己的動/靜分類。

**但它與 `V_ABS` 冗餘**：同一個目標的第一則訊息已經直接給了對地速度，誤差 0.33 m/s，
比一個 3 bit 的分類精細得多，而且每個槽都有。**沒有任何理由去解第二、三則訊息。**
