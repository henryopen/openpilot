...
## 6.4 這顆雷達到底給了什麼（2026-09-04 全面複驗，`radar_calibrate.py` / `radar_lateral.py`）

**先講結構：三十個位址不是三十個目標，是十個目標各三則訊息。** bus1 `0x238–0x255` 每個都在跑 33 Hz，
...
### `V_ABS` 是真的目標對地速度 —— 但**不要用距離微分去驗它**

⚠️ 這一節是我自己踩過的坑，記下來免得再犯：拿「±0.75 秒距離微分」當基準會得到
